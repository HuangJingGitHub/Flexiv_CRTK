...
flexiv::Robot_CRTK* robotPtr = nullptr;
const unsigned int k_jointSize = 7;
static std::vector<double> targetTcpPose;



/** Callback function for realtime periodic task */
bool moveRobot(flexiv::Robot_CRTK* robotPtr, double delta_x, double delta_y) {
...
int initRobotMain() {
    ...
    static flexiv::Robot_CRTK robot(robotIP, localIP);
    if (robot.isFault()) {
        robot.clearFault();
